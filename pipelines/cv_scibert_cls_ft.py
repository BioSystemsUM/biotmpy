...
from mlearning.dl import plot_roc_n_pr_curves, Bert_preprocessing
...
for train_index, test_index in kfold.split(x_train_df.to_numpy(), y_train_df.to_numpy()):
    ...
    if multiple_gpus:
        with strategy.scope():
            model = Bert_CLS_opt(dl_config, learning_rate=dl_config.learning_rate,static_bert=False, bert_name_or_path=scibert_path, bert_config=True)
    else:
        model = Bert_CLS_opt(dl_config, learning_rate=dl_config.learning_rate,static_bert=False, bert_name_or_path=scibert_path, bert_config=True)
