...
from src.biotmpy.preprocessing.dl_config import DLConfig
...
global model, dl_config
dl_config = DLConfig()
dl_config = dl_config.load(os.path.join(os.path.dirname(__file__),'best_model/biobert_lstm_ft_28/config.txt'))
...
def predict():
	...
	if pmids:
		try:
			...
			docs, pmids_not_found = pubmed_reader.pmids_to_docs(pmids, email, dl_config)
			if not docs:
		...
		try:
			...
			docs = pubmed_reader.term_to_docs(term, email, retmax=nmr_docs, dl_config=dl_config)
			if not docs:
				return render_template('result.html', tables=None)
		except:
		...
		try:
			...
			docs, docs_not_found = pubmed_reader.pdfs_to_docs(updloaded_files, email, dl_config)
			if not docs:
				return render_template('result.html', tables=None)
		except:
	...
	try:
		...
		x_test = Bert_preprocessing(x_test_df, dl_config=dl_config)
