model_name = 'embeddings_comparison_han_split'
...
for embedding in embeddings_list:
    ...
    for train_index, test_index in kfold.split(x_train_df.to_numpy(), y_train_df.to_numpy()):
        ...
        if multiple_gpus:
            with strategy.scope():
                model = Hierarchical_Attention_Context(config.embedding_matrix, config,
                                                       learning_rate=config.learning_rate,
                                                       seed_value=config.seed_value)
        else:
            model = Hierarchical_Attention_Context(config.embedding_matrix, config, learning_rate=config.learning_rate,
                                                   seed_value=config.seed_value)
