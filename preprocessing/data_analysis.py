...
def corpus_visualization(analysis_df, dim_red='tsne', feature_ext='tfidf', file_name = None, title=None, decompose_byint=50, random_state=None):
    ...
    if dim_red.lower() == 'tsne':
        visualizer = TSNEVisualizer(colormap=set_palette('sns_colorblind'), title=title, decompose_byint=decompose_byint, random_stateint=random_state, alphafloat=0.6)
    elif dim_red.lower() == 'umap':
        visualizer = UMAPVisualizer(colormap=set_palette('sns_colorblind'), title=title, random_stateint=random_state, alphafloat=0.6)
...
def plot_words_per_sentence(analysis_df, bins= 100, xTitle = 'Number of Words per Sentence', yTitle = 'Count', title='Text Length Distribution' , filename='text_len', threshold=None):
    ...
    print('Percentage of sentences with a number of words greater than or equal to {}: {:.2%}'.format(threshold, nmr_sentences_threshold/nmr_sentences))
...
def plot_labels_balance(analysis_df, filename='labels_balance'):
    ...
    plt.pie(d.values(), labels=d.keys(), autopct='%1.1f%%', shadow=True, startangle=90, colors=set_palette('colorblind') )
...
def plot_top_n_words(analysis_df, n=20, stop_words = None, file_name = 'top_words', n_grams = 1, yTitle = 'Count', title='Top 20 Words', fig_dim=(16,6)):
    ...
    print('Unique words: {} \n Non-Relevant: {} \n Relevant: {}'.format(len(unique_words_nr), unique_words_nr, unique_words_r))
