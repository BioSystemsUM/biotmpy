...
import re
import math
from data.term_lists import strong_mutation_list, weak_mutation_list, complete_mutation_list, \
...
def nmr_protein_names_n_ids(dataframe, documents):
    res = []
    protein_full_names_list = get_protein_names_n_ids('../examples_pipelines/term_lists/uniprot.tab')
    for d in documents:
        score = 0
        for sentence in d.sentences:
...
def nmr_protein_tokens(dataframe, documents):
    res = []
    prot_tokens = get_tokens_protein_names_n_ids('../examples_pipelines/term_lists/uniprot.tab')
    for d in documents:
        score = 0
        for sentence in d.sentences:
